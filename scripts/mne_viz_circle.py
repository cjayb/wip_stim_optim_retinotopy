...
def _plot_connectivity_circle_onpick(event, fig=None, axes=None, indices=None,
    ...
    if event.button == 1:  # left click
        ...
        # print('Node {:d}'.format(node), indices)
...
def plot_connectivity_circle(con, node_names, indices=None, n_lines=None,
                             ...
                             node_colors=None, node_radii=None,
                             node_heights=None, facecolor='black',
    ...
    node_radii : array, shape=(len(node_names,)) | None
        Array with node radii. If None, the nodes are placed
        on a circle of radius 10.
    node_heights : array, shape=(len(node_names,)) | None
        Array with node radial heights. If None, the nodes get a height of
        1/10th of the radius.
    ...
    if node_radii is not None:
        if len(node_radii) != n_nodes:
            raise ValueError('node_radii has to be the same length '
                             'as node_names')
    else:
        node_radii = 10. * np.ones(n_nodes)

    if node_heights is not None:
        if len(node_heights) != n_nodes:
            raise ValueError('node_heights has to be the same length '
                             'as node_names')
    else:
        node_heights = node_radii / 10.
    ...
    for pos, (i, j) in enumerate(zip(indices[0], indices[1])):
        # Start point
        t0, r0 = node_angles[i], node_radii[i]

        # End point
        t1, r1 = node_angles[j], node_radii[j]

        # Some noise in start and end point
        t0 += start_noise[pos]
        t1 += end_noise[pos]

        verts = [(t0, r0), (t0, node_radii[i] / 2.0),
                 (t1, node_radii[i] / 2.0), (t1, r1)]
    ...
    # node_bottom = node_radii - 1.0
    # node_bottom = 0.9 * np.min(node_radii) * np.ones(len(node_radii))
    node_bottom = 0.9 * node_radii
    bars = axes.bar(node_angles, node_heights, width=node_width,
                    bottom=node_bottom,
    ...
    for (name, angle_rad, angle_deg,
         rad, height) in zip(node_names, node_angles,
                             angles_deg, node_radii, node_heights):
        if angle_deg >= 270:
            ha = 'left'
        else:
        ...
        cur_rad = rad + height  # looks a little off, why?
        axes.text(angle_rad, cur_rad, name, size=fontsize_names,
